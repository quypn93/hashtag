...
var connectionString = "Server=112.78.2.36;Database=vir62982_tag;User Id=vir62982_user;Password=*1MAbonR?hu7saa7;Encrypt=false;";

using var connection = new SqlConnection(connectionString);
connection.Open();

// Get category IDs
int tiktokTipsCatId, hashtagTrendingCatId;

using (var cmd = connection.CreateCommand())
{
    cmd.CommandText = "SELECT Id FROM BlogCategories WHERE Slug = 'meo-tiktok'";
    tiktokTipsCatId = (int)cmd.ExecuteScalar();
}

using (var cmd = connection.CreateCommand())
{
    cmd.CommandText = "SELECT Id FROM BlogCategories WHERE Slug = 'phan-tich-trending'";
    hashtagTrendingCatId = (int)cmd.ExecuteScalar();
}

Console.WriteLine($"TikTok Tips Category ID: {tiktokTipsCatId}");
Console.WriteLine($"Hashtag Trending Category ID: {hashtagTrendingCatId}");
...
using (var cmd = connection.CreateCommand())
{
    ...
    cmd.Parameters.AddWithValue("@Title", "Cách Lên Xu Hướng TikTok 2026: 10 Bước Từ 0 Đến Triệu View");
    cmd.Parameters.AddWithValue("@Slug", "cach-len-xu-huong-tiktok-2026-10-buoc");
    cmd.Parameters.AddWithValue("@Excerpt", "Hướng dẫn chi tiết 10 bước để video TikTok của bạn lên xu hướng và đạt triệu view. Áp dụng ngay để viral!");
    cmd.Parameters.AddWithValue("@FeaturedImage", "https://images.unsplash.com/photo-1611162617474-5b21e879e113?w=1200&q=80");
    cmd.Parameters.AddWithValue("@Content", @"<h2>Bí Quyết Lên Xu Hướng TikTok 2026</h2>
...
<p><strong>Công thức thành công = Nội dung chất lượng + Hashtag đúng + Thời điểm đăng tốt + Kiên trì</strong></p>");
    cmd.Parameters.AddWithValue("@MetaTitle", "Cách Lên Xu Hướng TikTok 2026: 10 Bước Viral Triệu View");
    cmd.Parameters.AddWithValue("@MetaDesc", "Hướng dẫn chi tiết 10 bước để video TikTok lên xu hướng và viral triệu view. Bí quyết TikToker chuyên nghiệp!");
    cmd.Parameters.AddWithValue("@MetaKeywords", "cách lên xu hướng tiktok, viral tiktok, triệu view tiktok, xu hướng tiktok 2026");
    cmd.Parameters.AddWithValue("@Author", "TrendTag Team");
    cmd.Parameters.AddWithValue("@CategoryId", tiktokTipsCatId);
    ...
    cmd.ExecuteNonQuery();
    Console.WriteLine("Inserted: Cách Lên Xu Hướng TikTok 2026");
}
...
{
    cmd.CommandText = insertQuery;
    cmd.Parameters.AddWithValue("@Title", "Algorithm TikTok 2026: Cách FYP Hoạt Động Và Bí Quyết Lên For You Page");
    cmd.Parameters.AddWithValue("@Slug", "algorithm-tiktok-2026-cach-fyp-hoat-dong");
    cmd.Parameters.AddWithValue("@Excerpt", "Giải mã thuật toán TikTok 2026: Hiểu cách For You Page hoạt động để tối ưu video và tăng reach.");
    cmd.Parameters.AddWithValue("@FeaturedImage", "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=1200&q=80");
    cmd.Parameters.AddWithValue("@Content", @"<h2>Giải Mã Algorithm TikTok 2026</h2>
...
<p>Algorithm TikTok 2026 ưu tiên <strong>watch time</strong> và <strong>engagement</strong>.</p>");
    cmd.Parameters.AddWithValue("@MetaTitle", "Algorithm TikTok 2026: Cách FYP Hoạt Động - Bí Quyết Viral");
    cmd.Parameters.AddWithValue("@MetaDesc", "Giải mã thuật toán TikTok 2026: Hiểu cách For You Page hoạt động để tối ưu video và tăng triệu view.");
    cmd.Parameters.AddWithValue("@MetaKeywords", "algorithm tiktok, thuật toán tiktok, fyp tiktok, for you page, cách lên fyp");
    cmd.Parameters.AddWithValue("@Author", "TrendTag Team");
    cmd.Parameters.AddWithValue("@CategoryId", tiktokTipsCatId);
    cmd.Parameters.AddWithValue("@Status", "Published");
    cmd.Parameters.AddWithValue("@PublishedAt", DateTime.UtcNow.AddHours(-12));
    cmd.Parameters.AddWithValue("@ViewCount", 0);
    cmd.Parameters.AddWithValue("@CreatedAt", DateTime.UtcNow.AddHours(-12));
    cmd.ExecuteNonQuery();
    Console.WriteLine("Inserted: Algorithm TikTok 2026");
}
...
{
    cmd.CommandText = insertQuery;
    cmd.Parameters.AddWithValue("@Title", "Hashtag TikTok Việt Nam: Danh Sách 500+ Hashtag Theo Chủ Đề [2026]");
    cmd.Parameters.AddWithValue("@Slug", "hashtag-tiktok-viet-nam-danh-sach-500-hashtag");
    cmd.Parameters.AddWithValue("@Excerpt", "Tổng hợp 500+ hashtag TikTok phổ biến nhất tại Việt Nam, phân loại theo 16 chủ đề. Copy và sử dụng ngay!");
    cmd.Parameters.AddWithValue("@FeaturedImage", "https://images.unsplash.com/photo-1611162618071-b39a2ec055fb?w=1200&q=80");
    cmd.Parameters.AddWithValue("@Content", @"<h2>Danh Sách Hashtag TikTok Việt Nam Đầy Đủ Nhất</h2>
...
<li><a href='/phan-tich/theo-doi-tang-truong'>Theo Dõi Tăng Trưởng</a></li>
    ...
    cmd.Parameters.AddWithValue("@MetaTitle", "500+ Hashtag TikTok Việt Nam Theo Chủ Đề [2026] - Copy Ngay!");
    cmd.Parameters.AddWithValue("@MetaDesc", "Danh sách 500+ hashtag TikTok Việt Nam đầy đủ nhất, phân loại theo 16 chủ đề. Copy và sử dụng ngay!");
    cmd.Parameters.AddWithValue("@MetaKeywords", "hashtag tiktok việt nam, hashtag tiktok, danh sách hashtag, hashtag theo chủ đề");
    cmd.Parameters.AddWithValue("@Author", "TrendTag Team");
    cmd.Parameters.AddWithValue("@CategoryId", hashtagTrendingCatId);
    cmd.Parameters.AddWithValue("@Status", "Published");
    cmd.Parameters.AddWithValue("@PublishedAt", DateTime.UtcNow.AddHours(-6));
    cmd.Parameters.AddWithValue("@ViewCount", 0);
    cmd.Parameters.AddWithValue("@CreatedAt", DateTime.UtcNow.AddHours(-6));
    cmd.ExecuteNonQuery();
    Console.WriteLine("Inserted: Hashtag TikTok Việt Nam");
}
